...
local function quickchat_core(_, _, ask)
...
local function quickchat_visual_core(_, _, ask)
  if ask == true then
    local ok, input = pcall(vim.fn.input, "Quick Chat: ")
    if ok and input ~= "" then
...
local function prompt_action_core()
...
function M.quickchat(ask)
  local wrapped_fn = Nvim.DAPUI.with_layout_handling_when_dapui_open(quickchat_core)
  wrapped_fn(ask)
end

function M.quickchat_visual(ask)
  local wrapped_fn = Nvim.DAPUI.with_layout_handling_when_dapui_open(quickchat_visual_core)
  wrapped_fn(ask)
end
...
function M.prompt_action()
  ...
  local wrapped_fn = Nvim.DAPUI.with_layout_handling_when_dapui_open(prompt_action_core)
  wrapped_fn()
end
...
lvim.builtin.which_key.mappings.u['ki'] = { function() M.quickchat(false) end, "CopilotChat - Quick chat panel" }
lvim.builtin.which_key.vmappings.u['ki'] = { function() M.quickchat_visual(false) end,
  "CopilotChat - Quick chat panel" }
lvim.builtin.which_key.mappings.u['kw'] = { M.no_context_chat, "CopilotChat - No context chat" }

lvim.builtin.which_key.mappings.u['ka'] = { function() M.quickchat(true) end, "CopilotChat - Quick chat" }
lvim.builtin.which_key.vmappings.u['ka'] = { function() M.quickchat_visual(true) end, "CopilotChat - Quick chat" }

lvim.builtin.which_key.mappings.u['kk'] = { M.prompt_action, "CopilotChat Prompt Action" }
lvim.builtin.which_key.vmappings.u['kk'] = { M.prompt_action, "CopilotChat Prompt Action" }
